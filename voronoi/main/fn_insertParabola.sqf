/*
	...
	Function: VOR_fnc_insertParabola
...
if(PARABOLA_IS_NULL(_root)) exitWith {
	...
	private _newParabola = _point call VOR_fnc_newParabola;
...
if(_rootIsLeaf && _rootSite#POINT_Y - _point#POINT_Y < 1 ) exitWith {
	...
	private _leftParabola = _rootSite call VOR_fnc_newParabola;
	private _rightParabola = _point call VOR_fnc_newParabola;
	private _leftPtr = NEW_PTR(_leftParabola);
	private _rightPtr = NEW_PTR(_rightParabola);

	[_rootPtr, _leftPtr, true] call VOR_fnc_treeSetChild;
	[_rootPtr, _rightPtr, false] call VOR_fnc_treeSetChild;

	private _start = [(_pointX + _rootSiteX)/2, _height];
	_vertices pushBack _start;

	private _newEdge = if(_pointX < _rootSiteX) then {
		[_start, _rootSite, _point] call VOR_fnc_newEdge
	} else {
		[_start, _point, _rootSite] call VOR_fnc_newEdge
	};
...
private _parabolaPtr = [_rootPtr, _scanlineY, _pointX] call VOR_fnc_getParabolaByX;
...
private _start = [_pointX, [_site, _pointX, _scanlineY] call VOR_fnc_getY];
...
//Create half-edges
private _edgeLeft = [_start, _site, _point] call VOR_fnc_newEdge;
private _edgeRight = [_start, _point, _site] call VOR_fnc_newEdge;
...
private _p0 = _site call VOR_fnc_newParabola;
private _p1 = _point call VOR_fnc_newParabola;
private _p2 = _site call VOR_fnc_newParabola;
...
//Set right child
[_parabolaPtr, _p2Ptr, false] call VOR_fnc_treeSetChild;

//Set left child
private _innerParabola = [] call VOR_fnc_newParabola;
_innerParabola set [PARABOLA_EDGE, _edgeLeftPtr];
private _innerParabolaPtr = NEW_PTR(_innerParabola);
[_parabolaPtr, _innerParabolaPtr, true] call VOR_fnc_treeSetChild;

//Set left child's children
[_innerParabolaPtr, _p0Ptr, true] call VOR_fnc_treeSetChild;
[_innerParabolaPtr, _p1Ptr, false] call VOR_fnc_treeSetChild;

#ifdef DO_DEBUG
[_rootPtr, {_this}] call VOR_fnc_printTree;
#endif

//Do circle checks
[_p0Ptr, _eventQueue, _vertices, _scanlineY] call VOR_fnc_checkCircle;
[_p2Ptr, _eventQueue, _vertices, _scanlineY] call VOR_fnc_checkCircle;