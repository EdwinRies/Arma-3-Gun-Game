...
if(_width > 0 && _height > 0) then {
	...
	private PTR_MAIN = objNull call VOR_fnc_pointerNewContainer;
	...
	private _eventQueue = (
		_sites apply {
			[_x#POINT_Y, [[_x#0,_x#1], EVENT_TYPE_SITE] call VOR_fnc_newEvent]
		}
	) call VOR_fnc_heapNew;
		

#ifdef DO_DEBUG
	diag_log text "GENERATE - Sites:";
	private _queueDebug = +_eventQueue;
	while {_queueDebug call VOR_fnc_heapSize > 0} do {
		(_queueDebug call VOR_fnc_heapPop) params ["_key", "_event"];
	...
	private _i = 0;
	while {_eventQueue call VOR_fnc_heapSize > 0} do {
		(_eventQueue call VOR_fnc_heapPop) params ["_key", "_event"];
		_i = _i + 1;
#ifdef DO_DEBUG
...
#endif
				[_parabolaTree, _event#EVENT_POINT, _scanlineY, _height, _edges, _vertices, _eventQueue, _deletedEvents] call VOR_fnc_insertParabola;
				
#ifdef DO_DEBUG
				//diag_log text format ["GENERATE - POST-INSERT Tree size: %1", count _parabolaTree ];
				[_parabolaTree, {_this}] call VOR_fnc_printTree;
#endif
			} else {
#ifdef DO_DEBUG
				//diag_log text format ["GENERATE - PRE-REMOVE Tree size: %1", count _parabolaTree ];
#endif
				[_parabolaTree, _event, _scanlineY, _edges, _vertices, _eventQueue, _deletedEvents] call VOR_fnc_removeParabola;
	...
	//[_parabolaTree, {_this}] call VOR_fnc_printTree;
	[_parabolaTree, _width, _height, _vertices] call VOR_fnc_finishEdge;
	...
	{
		private _edge = _x; 
		[_edge, _width, _height] call VOR_fnc_boundEdge;
