...
#endif
private _leftParentPtr = [_bPtr, true] call VOR_fnc_treeGetParent;
private _rightParentPtr = [_bPtr, false] call VOR_fnc_treeGetParent;

#ifdef DO_DEBUG
diag_log text format ["_leftParentPtr = %1", _leftParentPtr];
diag_log text format ["_rightParentPtr = %1", _rightParentPtr];
#endif
private _aPtr = [_leftParentPtr, true] call VOR_fnc_treeGetLeafChild;
private _cPtr = [_rightParentPtr, false] call VOR_fnc_treeGetLeafChild;
...
if(_aValid && _cValid) then {
	...
	if(!(_aSite isEqualTo _cSite)) then {			
		...
		private _ins = [_leftParentEdgePtr, _rightParentEdgePtr, _vertices] call VOR_fnc_getEdgeIntersection;
		...
		if(!POINT_IS_NULL(_ins)) then {
			...
			if(_insY - _d < _lineY) then {
				...
				private _event = [_point, EVENT_TYPE_CIRCLE] call VOR_fnc_newEvent;
...
#endif
				[_eventQueue, _point#POINT_Y, _event] call VOR_fnc_heapInsert;
			};
		};
	};
};